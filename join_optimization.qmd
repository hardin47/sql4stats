...
### Results
#### Plot of Execution Times by Order and Algorithm
...
```{r}
dbGetQuery(con, paste("EXPLAIN ANALYZE", sql_bad_order))
```

It's important to recognize that query plans need to be read from bottom to top--that is, the step labeled "1" in the plan is actually the last portion of the query. 
