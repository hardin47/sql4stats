...
For more information on these strategies, see the <a href="joins.html">Joins page in the Theoretical Foundations section of the site</a>.
...
First, we connect to the database. A local PostgreSQL server connection is ideal for this simulation as it will have the least performance variability. Note: to run the code below, you need to have a local PostgreSQL server on your personal computer. On a Mac, the installation might be as straightforward as running `brew install postgresql@16` in the terminal. We use the default / existing "postgres" database.
...
You will need to create a table in your local MySQL server called "stats" and add the STATS-CEB data from <a href = "https://github.com/Nathaniel-Han/End-to-End-CardEst-Benchmark" target = "_blank">https://github.com/Nathaniel-Han/End-to-End-CardEst-Benchmark</a>.  
...
auto_exec <- auto_plan |>
  str_subset("Execution Time") |>
  str_extract("[0-9.]+") |>
  as.numeric()
auto_plan_time <- auto_plan |>
  str_subset("Planning Time") |>
  str_extract("[0-9.]+") |>
  as.numeric()
...
forced_exec <- forced_plan |>
  str_subset("Execution Time") |>
  str_extract("[0-9.]+") |>
  as.numeric()
forced_plan_time <- forced_plan |>
  str_subset("Planning Time") |>
  str_extract("[0-9.]+") |>
  as.numeric()
